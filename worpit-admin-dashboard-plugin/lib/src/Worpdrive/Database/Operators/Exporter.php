...
class Exporter {
	...
	public function buildTableDataStructureRows( string $table ) :void {
		...
		if ( $this->cfg->has( 'single-transaction' ) ) {
			if ( $DB->doSql( "SET GLOBAL TRANSACTION ISOLATION LEVEL REPEATABLE READ; START TRANSACTION;" ) === false ) {
		...
		$colResults = $DB->getResults( sprintf( "SHOW FULL COLUMNS FROM `%s`", $table ) );
		...
		foreach ( $columns as $field => $type ) {
			...
			elseif ( $this->cfg->has( 'hex-blob' ) && \preg_match( '#^blob|longblob|mediumblob|tinyblob|binary|varbinary#i', $type ) ) {
				...
				$rowValues[] = \is_null( $row[ $field ] ) ?
					'NULL' : "'".\ICWP_APP_WpDb::GetInstance()->loadWpdb()->_real_escape( $row[ $field ] )."'";
			}
		}
		return $rowValues;
	}
