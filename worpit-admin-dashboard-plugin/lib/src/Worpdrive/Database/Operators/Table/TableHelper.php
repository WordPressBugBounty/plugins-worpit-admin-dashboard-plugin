...
class TableHelper {
	...
	public function showColumns() :array {
		if ( $this->columns === null ) {
			$colResults = $this->DB()->getResults( sprintf( "SHOW FULL COLUMNS FROM `%s`", esc_sql( $this->table ) ) );
	...
	public function showCreate() :array {
		$tableCreateSQL = $this->DB()->getResults( sprintf( 'SHOW CREATE TABLE `%s`', esc_sql( $this->table ) ) );
	...
	private function getStandardPrimaryKeyForTable() :?string {
		$unPrefixed = \preg_replace( sprintf( '#^%s#i', $this->DB()->getPrefix() ), '', $this->table, 1 );
