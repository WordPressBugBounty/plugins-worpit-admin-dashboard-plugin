...
/**
 * 20250102
 * Dropped-in from https://github.com/deliciousbrains/wp-background-processing/blob/1.4.0/classes/wp-background-process.php
 */
abstract class ICWP_WP_Background_Process extends \ICWP_WP_Async_Request {
	/**
	...
	public function schedule_cron_healthcheck( $schedules ) {
		...
		if ( 1 === $interval ) {
			$display = __( 'Every Minute', 'worpit-admin-dashboard-plugin' );
		} else {
			/* translators: ignore this as it's part of another plugin */
			$display = sprintf( __( 'Every %d Minutes', 'worpit-admin-dashboard-plugin' ), $interval );
		}
